...
export default function Article({id, name, claps, bookmark, handleClapClick, handleBookmarkClick, handleStarClick, handleFollowClick, didClap, user}) {
  ...
  return(
    <div key={id} className={C.ArticleContainer}>
      <img src={myImage} alt={name} className={C.ArticleImageItem} />
      <h3 className={C.ArticleTitleItem}>{name}</h3>
      <div className={C.AuthorAndLikesItem}>
        <div className={C.AuthorAndLikesContainer}>
          <User
          className={C.AuthorItem}
            key={user.id}
            id={user.id}
            name={user.name}
            date={user.date}
          ...
          />
          <div className={C.LikesItem + ' ' + C.LikesContainer}>
            {didClap ? clapFilled : clapUnfilled}
            <div className={C.ClapsNumberItem}>{claps}</div>
            <span className={C.SeparatorItem}></span>
